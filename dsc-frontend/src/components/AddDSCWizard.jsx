...
function StepDetails({ cert, clients, defaultClientId, onNext, onBack }) {
  const [f, setF] = useState({ label: '', location: '', assigned_to: '', notes: '', client_id: defaultClientId || 'own', token_pin: '', cert_password: '' });
    ...
    <div className="p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">
              Token PIN <span className="text-slate-400 font-normal">— optional</span>
            </label>
            <div className="relative">
              <input
                ...
                className={`${inputCls} pr-9 font-mono tracking-widest placeholder:font-sans placeholder:tracking-normal`}
              />
              <button type="button" onClick={() => setShowPin(p => !p)} tabIndex={-1}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">
              Cert Password <span className="text-slate-400 font-normal">— optional</span>
            </label>
            <div className="relative">
              <input
                type={showPass ? 'text' : 'password'}
                name="cert_password"
                value={f.cert_password}
                onChange={set}
